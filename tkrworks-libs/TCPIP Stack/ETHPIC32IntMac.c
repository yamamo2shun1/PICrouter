...
// RX buffers
#ifdef USE_SPI_SRAM//syama
//syama
#include "osc.h"

static unsigned char _OneRxBuffer[EMAC_RX_BUFF_SIZE];
static unsigned char* getRxBuffers(BYTE index)
{
    memset(_OneRxBuffer, 0, MAX_PACKET_SIZE);
    readBytesFromSRAM((MAX_BUF_SIZE * MAX_PACKET_SIZE) + index * EMAC_RX_BUFF_SIZE, _OneRxBuffer, EMAC_RX_BUFF_SIZE);
    return _OneRxBuffer;
}
static void setRxBuffers(WORD index, unsigned char* buffer)
{
    writeBytesToSRAM((MAX_BUF_SIZE * MAX_PACKET_SIZE) + index * EMAC_RX_BUFF_SIZE, buffer, EMAC_RX_BUFF_SIZE);
}
#else
...
{
#ifdef USE_SPI_SRAM //syama
    int avlblRxBuffs = (EMAC_RX_DESCRIPTORS * EMAC_RX_BUFF_SIZE) / EMAC_RX_BUFF_SIZE - EthDescriptorsGetRxUnack();
    return avlblRxBuffs * (EMAC_RX_BUFF_SIZE);
#else
