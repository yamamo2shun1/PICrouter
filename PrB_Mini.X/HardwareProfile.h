...
#define HARDWARE_PROFILE_H
    ...
    // LED_PAD_16
    //#define LD16_LOAD(state) state
#if 0
    #define LD16_LOAD(state) LATCbits.LATC13 = state;
    #define SR_SL(state)     LATCbits.LATC14 = state;
    #define SR_CLK(state)    LATFbits.LATF0  = state;
    #define SR_QH()          PORTFbits.RF1
#else
    #define LD16_LOAD(state) LATBbits.LATB13 = state;
    #define SR_SL(state)     LATBbits.LATB12 = state;
    #define SR_CLK(state)    LATBbits.LATB11 = state;
    #define SR_QH()          PORTBbits.RB10
#endif

    // LED_PAD_64
    //#define LD64_LOAD(state) state
#if 0
    #define LD64_LOAD(state) LATCbits.LATC13 = state;
    ...
    // LED_ENC
    //#define LD32_LOAD(state) state
#if 1
    #define LD32_LOAD(state) LATCbits.LATC13 = state;
    //test #define RE_SW()          PORTCbits.RC14
    //test #define RE_A()           PORTFbits.RF0
    //test #define RE_B()           PORTFbits.RF1
    // test for EMS22A1050
    #define RE_CS(state)     LATCbits.LATC14 = state;
    //#define RE_CLK(state)    LATFbits.LATF0 = state;
    #define RE_CLK_TGL()     LATFbits.LATF0 = !LATFbits.LATF0;
    #define RE_CLK(state)    LATFbits.LATF0 = state;
    #define RE_DO()          PORTFbits.RF1
#endif
